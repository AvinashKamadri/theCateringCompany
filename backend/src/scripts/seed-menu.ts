...
interface MenuItem {
  ...
  tags?: string[];
  is_upsell?: boolean;
  // Explicit component dishes for a platter/combo menu item.
  dishes?: string[];
  // Option-choice groups (e.g. "Soup (Pick 2)": [...]) — each option becomes a dish.
...
interface MenuCategory {
  ...
  sort_order: number;
  items: MenuItem[];
  // If true, seed no dishes for this category (bar packages, cake variants, etc.)
  skip_dishes?: boolean;
}
...
const menuData: MenuCategory[] = [
  ...
  // ── I. Hors D'oeuvres ─────────────────────────────────────────────────
  // Single-item menu items → 1 dish per menu_item (name = menu_item name).
  ...
  // ── III. Main Menus — platters with explicit dish component lists ─────
    ...
    items: [
      {
        ...
        dishes: ['Carved Prime Rib', 'Roasted Salmon', 'Roasted Potatoes', 'Wild Rice', 'Glazed Carrots', 'Grilled Asparagus', 'Dinner Rolls'],
      },
      {
        name: 'Chicken & Ham',
        ...
        dishes: ['Grilled Chicken Breast', 'Mango Glazed Ham', 'Mashed Potatoes', 'Rice Pilaf', 'Buttered Corn', 'Green Beans', 'Dinner Rolls'],
      },
      {
        ...
        dishes: ['Chicken Piccata', 'Red Wine Braised Beef Roast', 'Vegetable Farfalle', 'Long Grain Buttered Rice', 'Roasted Mixed Veggies', 'Green Beans', 'Dinner Rolls'],
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        dishes: ['BBQ Beef Brisket', 'Beer Can Chicken', 'Mac & Cheese', 'Baked Beans', 'Coleslaw', 'Pasta Salad', 'Potato Salad'],
      },
      {
        name: 'Pork & Chicken',
        ...
        dishes: ['Pulled BBQ Pork', 'Pulled BBQ Chicken', 'Mac & Cheese', 'Baked Beans', 'Coleslaw', 'Pasta Salad', 'Potato Salad'],
      },
    ],
  },
  {
    ...
    items: [
      {
        name: 'Burger Bar',
        description: 'Burgers (handmade) w/ Brioche Buns, Beer Can Chicken Breast. Toppings Bar: Mushrooms, Grilled Onions, Pickled Red Onion, Pickles, Lettuce, Tomato, Bacon, Assorted Sauces and Cheeses. Sides: Mac & Cheese, Roasted Red Potato, Seasonal Spring Greens Salad, Caprese Platter, Watermelon Salad.',
        unit_price: 23.99, price_type: 'per_person',
        dishes: ['Handmade Burgers', 'Beer Can Chicken Breast', 'Mac & Cheese', 'Roasted Red Potato', 'Seasonal Spring Greens Salad', 'Caprese Platter', 'Watermelon Salad'],
      },
      {
        ...
        dishes: ['Garden Salad', 'Crispy Fried Chicken', 'Smoked Sausage', 'Mac & Cheese', 'Mashed Potatoes', 'Southern Style Green Beans', 'Buttered Corn Kernel', 'Corn Bread'],
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        unit_price: 27.99, price_type: 'per_person',
        dishes: ['Carne Asada', 'Chili Lime Chicken', 'Spanish Rice', 'Bandito Black Beans', 'Peppers & Onions', 'Pico De Gallo'],
      },
      {
        ...
        unit_price: 23.99, price_type: 'per_person',
        dishes: ['Braised Spanish Beef', 'Braised Chili Chicken', 'Pinto Beans', 'Cilantro Lime Rice'],
      },
      {
        name: 'Mediterranean Bar',
        description: 'Hummus Bar (all homemade): Roasted Garlic, Sundried Tomato, Original. Toppings: Ground Lamb, Grilled Mediterranean Chicken, Roasted Vegetables, Feta Cheese, Roasted Chickpeas, Olives, Pita Bread.',
        unit_price: 23.49, price_type: 'per_person',
        dishes: ['Hummus Bar', 'Ground Lamb', 'Grilled Mediterranean Chicken', 'Roasted Vegetables', 'Pita Bread'],
      },
      {
        name: 'Souvlaki Bar',
        description: 'Proteins: Chicken Souvlaki, Pork Souvlaki. Sides/Toppings: Roasted Greek Potatoes, Roasted Mixed Vegetables, Green Beans, Pita Bread, Tzatziki, Feta Cheese.',
        unit_price: 21.49, price_type: 'per_person',
        dishes: ['Chicken Souvlaki', 'Pork Souvlaki', 'Roasted Greek Potatoes', 'Roasted Mixed Vegetables', 'Green Beans', 'Pita Bread', 'Tzatziki'],
      },
      {
        name: 'Marsala Menu',
        ...
        unit_price: 25.99, price_type: 'per_person',
        dishes: ['Chicken Marsala', 'Roasted Cod', 'Vegetable Farfalle', 'Fettuccini', 'Roasted Mixed Veggies', 'Green Beans', 'Dinner Rolls'],
      },
      {
        name: 'Ravioli Menu',
        ...
        unit_price: 31.99, price_type: 'per_person', tags: ['premium'], is_upsell: true,
        dishes: ['Garden Salad', 'Grilled Chicken with Wild Mushroom Beurre Blanc', 'Roasted Salmon', 'Truffle Ravioli', 'Wild Rice', 'Sauteed Zucchini and Tomatoes', 'Roasted Asparagus', 'Dinner Rolls'],
      },
      {
        ...
        unit_price: 21.49, price_type: 'per_person',
        dishes: ['Caesar Salad', 'Grilled Chicken Breast', 'Sliced Italian Sausage', 'Pesto Penne Alfredo', 'Green Beans', 'Honey Glazed Carrots', 'Dinner Rolls'],
      },
    ],
  },

  // ── Soup / Salad / Sandwich — each option = a dish ────────────────────
    ...
    items: [
      {
        ...
        description: 'Pick 2 soups, 2 salads, and 2 sandwiches/wraps from our full selection.',
        unit_price: 21.95, price_type: 'per_person',
        options: {
          'Soup (Pick 2)':             ['Broccoli Cheddar', 'Loaded Potato', 'Tomato Basil Bisque', 'Chicken Tortilla', 'Vegetable Minestrone', 'Clam Chowder', 'French Onion', 'Chicken Noodle', 'Traditional Chili', 'Chicken Chili'],
          'Salad (Pick 2)':            ['Caesar Salad', 'Cobb Salad', 'Greek Salad', 'Southwest Salad', 'Potato Salad', 'Coleslaw', 'Seasonal Greens Salad', 'Bacon and Blue Cheese Salad', 'Pasta Salad', 'Sweet Kale Salad w/ Bacon and Cranberries', 'Design Your Own Salad'],
          'Sandwich / Wrap (Pick 2)':  ['Gourmet Grilled Cheese', 'Corned Beef Reuben', 'Pulled Pork Cuban', 'Avocado BLT', 'Pesto Chicken', 'Turkey Club', 'Roasted Vegetable and Hummus Wrap', 'Philly Cheese Steak', 'Build Your Own Sandwich'],
        },
      },
    ],
  },

  // ── Mini Desserts — each menu_item = 1 dish ───────────────────────────
...
async function seedMenu() {
  ...
  for (const categoryData of menuData) {
    try {
      ...
      const category = await prisma.menu_categories.upsert({
        ...
        update: { sort_order: categoryData.sort_order, active: true },
      });
      categoriesCreated++;
      ...
      for (const itemData of categoryData.items) {
        try {
          const existing = await prisma.menu_items.findFirst({
            where: { category_id: category.id, name: itemData.name },
            select: { id: true },
          });
          if (existing) {
          ...
          errors++;
          console.error(`  Error upserting item "${itemData.name}":`, (error as Error).message);
        }
      }

      console.log(`  Upserted ${categoryData.items.length} items`);
    } catch (error) {
      errors++;
      console.error(`Error upserting category "${categoryData.name}":`, (error as Error).message);
    }
  }
  ...
  console.log(`Categories upserted: ${categoriesCreated}/${menuData.length}`);
  console.log(`Menu items upserted: ${itemsCreated}`);
  console.log(`Errors: ${errors}`);
  console.log('='.repeat(50));
}

async function seedPricingPackages() {
  console.log('\nUpserting pricing packages...\n');
  ...
  for (const pkg of packages) {
    try {
      const existing = await prisma.pricing_packages.findFirst({ where: { name: pkg.name }, select: { id: true } });
      if (existing) {
        await prisma.pricing_packages.update({ where: { id: existing.id }, data: { ...pkg, active: true, currency: 'USD' } });
      } else {
      ...
      console.error(`  Error upserting "${pkg.name}":`, (error as Error).message);
    }
  }

  console.log(`\nUpserted ${packagesCreated}/${packages.length} pricing packages\n`);
}

function dishesForMenuItem(item: MenuItem, category: MenuCategory): string[] {
  if (category.skip_dishes) return [];
  if (item.dishes && item.dishes.length > 0) return item.dishes;
  if (item.options) return Object.values(item.options).flat();
  // Default: 1 dish = 1 menu_item (single-item categories like hors d'oeuvres, desserts, platters).
  return [item.name];
}

async function seedDishes() {
  console.log('\nSeeding dishes (structured)...\n');

  // Rebuild links + orphan dishes from the garbage comma-split pass. Dishes
  // with real manual ingredient work (dish_ingredients rows) are preserved.
  const deletedLinks = await prisma.menu_item_dishes.deleteMany({});
  const orphans = await prisma.dishes.findMany({
    where: { dish_ingredients: { none: {} } },
    select: { id: true },
  });
  if (orphans.length > 0) {
    await prisma.dishes.deleteMany({ where: { id: { in: orphans.map((o) => o.id) } } });
  }
  console.log(`  Cleared ${deletedLinks.count} old links and ${orphans.length} orphan dishes.`);
  ...
  for (const category of menuData) {
    for (const item of category.items) {
      const dishNames = dishesForMenuItem(item, category);
      if (dishNames.length === 0) continue;

      const menuItem = await prisma.menu_items.findFirst({
        where: { name: item.name, menu_categories: { name: category.name } },
        select: { id: true },
      });
      ...
      for (let i = 0; i < dishNames.length; i++) {
        ...
        try {
          ...
          await prisma.menu_item_dishes.create({
            data: { menu_item_id: menuItem.id, dish_id: dish.id, sort_order: i },
          });
          linksCreated++;
        } catch (error) {
          console.error(`  Error linking dish "${dishName}" → "${item.name}":`, (error as Error).message);
        }
      }
    }
  }
  ...
  console.log('='.repeat(50));
  console.log(`Dishes upserted (including repeats): ${dishesUpserted}`);
  console.log(`Menu-item ↔ dish links created: ${linksCreated}`);
  console.log('='.repeat(50));
}
