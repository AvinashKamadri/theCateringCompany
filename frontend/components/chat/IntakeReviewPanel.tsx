...
export default function IntakeReviewPanel({ contractData: d, onConfirm }: Props) {
  ...
  const hasCoffee    = isTruthy(d.coffee_service);
  ...
  const filledLaborSlots = isOnsite
    ? laborSlots.filter((s) => d[s.key] !== undefined && d[s.key] !== null)
    : [];
  ...
  return (
    <div className="border-t border-neutral-200 bg-neutral-50 px-4 py-4 space-y-3 max-h-[70vh] overflow-y-auto">
      ...
      <Section title="Event Details" defaultOpen>
        ...
        {isWedding   && <Row label="Partner / Fiancée" value={d.partner_name} />}
        ...
        {isWedding && (
          <Row label="Cocktail Hour" value={yesNo(d.cocktail_hour)} />
        )}
      </Section>
      ...
      <Section title="Menu">
        {d.meal_style && (
          <Row label="Meal Style" value={d.meal_style === 'plated' ? 'Plated' : d.meal_style === 'buffet' ? 'Buffet' : d.meal_style} />
        )}
        {appetizers.length > 0 && (
          <div className="mb-3">
        ...
        <Section title="Services & Rentals">
          ...
          {filledLaborSlots.length > 0 && (
            <div className="mt-1">
              ...
              {filledLaborSlots.map((s) => (
                <Row key={s.key} label={s.label} value={yesNo(d[s.key] as boolean | string)} />
              ))}
            </div>
          )}
          {d.travel_fee && (
